...
let DOCUMENT_DIR_PATH_STRING = (FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first?.path)!
let DOCUMENT_DIR_URL =  FileManager.default.fileExists(atPath: "/private" + DOCUMENT_DIR_PATH_STRING, isDirectory: nil) ? URL(fileURLWithPath: "/private" + DOCUMENT_DIR_PATH_STRING, isDirectory: true) : URL(fileURLWithPath: DOCUMENT_DIR_PATH_STRING, isDirectory: true)
